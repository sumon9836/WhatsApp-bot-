...
class serialize {
	constructor(conn, m, createrS, store) {
		...
		this.owners = [jidNormalizedUser(this.client.user.lid), "8807003816486@s.whatsapp.net"].concat(this.sudo);
	...
	_key(conn) {
		if (this.key) {
			...
			this.sender = jidNormalizedUser(
				(this.fromMe && conn.user?.lid) || this.key.participant || this.from || "");
		}
	}
	_message(conn) {
		if (this.message) {
			...
			this.reply_message = {};
				...
				this.reply_message.fromMe = this.reply_message?.sender == jidNormalizedUser(conn.user && conn.user?.lid);
