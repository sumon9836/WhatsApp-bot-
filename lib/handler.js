...
async function isBotAdmin(m, conn) {
		if (!m.key?.remoteJid?.endsWith('@g.us') && !m.jid?.endsWith('@g.us')) return false; // Not a group
...
async function isAdmin(m, conn) {
		...
		console.log('m.jid:', m.jid);

		if (!m.key?.remoteJid?.endsWith('@g.us') && !m.jid?.endsWith('@g.us')) {
				console.log('Not a group - returning false');
				return false;
		}
		...
		try {
				const groupMetadata = await client.groupMetadata(groupJid);
				console.log('groupMetadata participants count:', groupMetadata.participants.length);

				const senderJid = jidNormalizedUser(m.key?.participant || m.sender || m.key?.remoteJid);
				console.log('senderJid:', senderJid);

				const sender = groupMetadata.participants.find(p => jidNormalizedUser(p.id) === senderJid);
				console.log('sender participant found:', !!sender);
				console.log('sender admin status:', sender?.admin);
...
module.exports = {cutAudio,cutVideo,toAudio,isAccess,toPTT,isAdmin,isBotAdmin,isBot,getCompo,getDate,parsedJid,PREFIX,mode,extractUrlsFromString,getJson,isIgUrl,getUrl,isNumber,MediaUrls};